...
public class Tetromino extends Cell{
	 ...
	 */
	public Cell[] spin() {
		//判断是否调用对象为D型对象,其他方法在类中重写toString(),equals()等
		if(this.getClass().equals(new D().getClass()))
			return null;
...
class J extends Tetromino {
	/*
	 * 实现cells中的四个格子的特殊排列
	 * Tetris.J代表特例图片（颜色）——来自Tetris类中加载的静态资源
	 */
	public J() {
...
class D extends Tetromino {
	public D() {
		cells[0] = new Cell(0, 5, Tetris.D);
		cells[1] = new Cell(0, 6, Tetris.D);
		cells[2] = new Cell(1, 5, Tetris.D);
		cells[3] = new Cell(1, 6, Tetris.D);
	}
}
